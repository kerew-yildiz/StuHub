...
| V2.3 | Export + Arşiv | ✅ TAMAM | (bu oturum) | Not MD/PDF, Anki `.apkg` (stdlib), CSV, dönem arşivi export/import (manifest v1) — backend servisleri + router + frontend butonları (Not MD İndir, set/ders Anki+CSV, TermDetailPage arşiv indirme, TermsPage arşiv import) |
...
| V2.5 | Essay Değerlendirici | ✅ TAMAM | (bu oturum) | `essay_service` (0–100 rubrik, güven eşiği, boş metin deterministik) + `essays/grade` uçları + CoursePage "Ödev Değerlendir" sekmesi (EssayGraderForm + geçmiş) |
...
| FB1 | Geri Bildirim Turu v2 (5 madde) | ✅ TAMAM | (bu oturum) | 1) Not üretimi üç kademeli kaynak zinciri (kitap → web arama yedeği [ddgs] → slayt temelli tam not + deterministik son güvence; "eksik not" kaldırıldı) + atıf başına LLM onayı kaldırıldı (perf: 485→0 citation_confirm) + SDK timeout 120sn + alıntı ayıklama düzeltmesi; 2) flashcard atıf yumuşatması (atıfsız konu uyarıyla kart üretir) — kart/rehber zinciri düzeldi; 3) progress göstergeleri açılır-kapanır slider; 4) chapter'a çoklu sunum (slide_no sürekliliği + "Yeni sunum ekle" UI); 5) dönem/ders/chapter düzenleme (chapter PUT + düzenleme UI'ları). Kapılar: 196 pytest + 30 vitest; CANLI test (Learning dersi): not 435sn → 212 kart → özet (13 terim) → kavram haritası (16 düğüm) |
