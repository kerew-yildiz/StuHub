...
async def _course_exists(course_id: int) -> bool:
    db = await get_db()
    try:
...
async def _create_material(
    course_id: int, mtype: str, filepath: str, job_kind: str
) -> dict:
    db = await get_db()
    try:
        cursor = await db.execute(
            "INSERT INTO materials (course_id, type, filepath) VALUES (?, ?, ?)",
            (course_id, mtype, filepath),
        )
        await db.commit()
        ...
        cursor = await db.execute(
            "SELECT id, course_id, type, filepath, created_at FROM materials WHERE id = ?",
            (material_id,),
        )
    ...
    job_id = await create_indexing_job(course_id, material_id, kind=job_kind)
    await process_pending_jobs()
    return {**dict(row), "job_id": job_id}


@router.post("/courses/{course_id}/media/youtube", status_code=201)
async def ingest_youtube(course_id: int, payload: YoutubeIn) -> dict:
    ...
    if not await _course_exists(course_id):
        raise HTTPException(status_code=404, detail="Ders bulunamadı")
    return await _create_material(course_id, "youtube", payload.url.strip(), "index")


@router.post("/courses/{course_id}/media/text", status_code=201)
async def ingest_text(course_id: int, payload: TextIn) -> dict:
    ...
    return await _create_material(course_id, "text", str(stored), "index")
