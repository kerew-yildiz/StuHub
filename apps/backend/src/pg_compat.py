...
- `INSERT INTO ...` ifadelerine (zaten `RETURNING` yoksa) `RETURNING id` eklenir;
  ...
  `id` kolonu olmayan tabloda bu deneme SAVEPOINT içinde yapılır, böylece
  UndefinedColumnError dış işlemi abort durumunda bırakmaz.
    ...
    **İşlem (transaction) semantiği.** asyncpg varsayılan olarak autocommit'tedir;
    ...
    async def execute(self, sql: str, params: tuple = ()) -> PgCursor:
        ...
        if is_insert and not has_returning:
            ...
            # Deneme SAVEPOINT içinde yapılır: Postgres'te hata veren ifade transaction'ı
            # abort durumuna geçirir ve savepoint'e dönülmeden sonraki hiçbir komut kabul
            # edilmez — savepoint'siz `except` dalındaki ikinci execute
            # InFailedSQLTransactionError ile düşüyordu (bkz. KOK-NEDEN-K1).
            await self._raw.execute("SAVEPOINT pgc_ret")
            try:
                row = await self._raw.fetchrow(attempt, *params)
                await self._raw.execute("RELEASE SAVEPOINT pgc_ret")
                ...
                await self._raw.execute("ROLLBACK TO SAVEPOINT pgc_ret")
