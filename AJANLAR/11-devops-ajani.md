...
3. Windows yerel çalıştırmasını doğrula: `uv run uvicorn` + `pnpm run dev` uçtan uca; tek komut script'i sağla.
4. Dokümantasyonu yaz: kurulum (Python/uv + Node/pnpm, LibreOffice opsiyonel), ilk kullanım (API anahtarı girişi, dönem/ders/chapter akışı), sorun giderme.
...
- Araçlar: pwsh (git, pnpm, uv, gitleaks), job_*/terminal_* (dev server yönetimi)
